class SubmissionsController < ApplicationController
  ...
  def last_submission
    @problem = Problem.exists?(params[:prob_id])
    if !@problem
      flash[:alert] = "This problem was deleted by admin"
    else
      @submission = current_user.submissions.where(problem_id: params[:prob_id]).last if current_user.submissions
      if !@submission
        flash[:alert] = "you have 0 submissions for this problem"
      end
    end
  end

  def submit
    @problem = Problem.exists?(params[:problem_id])
    if @problem
      # get ide-form data
      ...
      flash.now[:alert] = "This problem was deleted by admin"
      render partial: 'problems/response'
    end
  end
